...
export const EditApplication = () => {
  ...
  useEffect(() => {
    setCompanyName(applicationDetails?.data?.company_name);
    setJobTitle(applicationDetails?.data?.job_title);
    setJobLocation(applicationDetails?.data?.job_Location);
    setSalary(applicationDetails?.data?.salary);
    setLink(applicationDetails?.data?.link);
    setJobDescription(applicationDetails?.data?.job_Description);
  }, [applicationDetails]);

  const { mutate: updateApplication } = useUpdateApplicationHook({
    onSuccess: (response) => {
      console.log("gats", response);
      messageApi.open({
        type: "success",
    ...
    <>
      ...
      <div className="h-full flex flex-col items-center pb-20 overflow-auto">
        <div className="w-3/4 flex flex-col">
          <h1 className="text-xl font-bold py-4">Edit Application</h1>
          <form
          ...
          >
            ...
            <div className="flex justify-end">
              <button
                type="submit"
                className={`bg-[#2E72D8] w-fit px-3 py-1 text-white rounded-2xl hover:bg-[#2e83d8] ${
